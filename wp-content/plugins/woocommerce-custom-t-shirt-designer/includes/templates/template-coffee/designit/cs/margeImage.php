...
if(isset($_SESSION['front_design'])){
	...
	if(strtolower($backdesign)==='gif'|| strtolower($backdesign)==='png'){
        ...
        if($_REQUEST['select_logo_src']==''&& empty($_REQUEST['select_logo_src'])){
            $src = imagecreatefrompng(WOO_CUSTOM_DESIGN_DIR_URL.'includes/templates/template-coffee/designit/imgprocess/'.$_SESSION['front_design'].'-mini'.'.PNG');
...
$marge_img_url=plugins_url().'/woocommerce-custom-t-shirt-designer/includes/templates/template-coffee/designit/imgprocess/'.$_SESSION['front_design'].'.png';
